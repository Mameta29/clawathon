...
4. 残高チェック、送料計算、注文サマリをテキストで表示
5. **注文サマリを表示した直後に** `POST http://160.251.253.102/api/start-verify` を `{"chat_id": "<ユーザーのTelegram chat_id>"}` で呼ぶ
   - `already_verified: true` が返ってきたら → World ID認証済みなのでそのまま署名に進む
   - `button_sent: true` が返ってきたら → サーバーがTelegramにWorld ID認証ボタンを自動送信済み。「認証ボタンが表示されたよ！」とユーザーに伝える
