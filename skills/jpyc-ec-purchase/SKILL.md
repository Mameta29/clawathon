...
4. 残高チェック、送料計算、注文サマリ提示 + **World ID認証ボタンを同時に表示**:
   - まず `POST http://160.251.253.102/api/start-verify` に `{"chat_id": "<ユーザーのchat_id>"}` を送信して `verify_url` を取得
   - Telegram Bot API で注文サマリと認証ボタンを1つのメッセージで送信:
     ```bash
     curl -s -X POST "https://api.telegram.org/bot8700870370:AAE40wHFnYmQpmA_imrdy_ZZ8ptkK4jQ_QQ/sendMessage" \
       -H "Content-Type: application/json" \
       -d '{
         "chat_id": "<chat_id>",
         "text": "📋 注文サマリ\n\n• 商品: ...\n• 合計: ... JPYC\n• 配送先: ...\n\n👇 下のボタンから認証すれば購入完了！",
         "reply_markup": {
           "inline_keyboard": [[{"text": "🌐 World IDで認証して購入する", "url": "<verify_url>"}]]
         }
       }'
     ```
   - ユーザーがボタンを押して認証完了すると、サーバーが自動でTelegramに通知 + キーボードボタン「署名して購入を完了する」が表示される
5. ユーザーがキーボードボタンを押す → 署名生成 (ows) → 署名提出 (API)
6. 注文番号 + 「mameta.zk@gmail.com 宛に購入完了メールが届きます」と返信

**重要**:
- 注文サマリと認証ボタンは同時に表示する。「進めていい？」等の追加確認は不要
- World IDの認証 = 購入の意思確認。認証後は追加の署名確認を求めない
- 認証完了後にキーボードボタンが表示されるので、それを押したら即座に署名→購入実行
