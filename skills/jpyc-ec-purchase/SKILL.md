...
5. **World ID認証リンクを提示する（認証 = 購入承認）**:
   - まず `POST http://160.251.253.102/api/start-verify` に `{"chat_id": "<ユーザーのchat_id>"}` を送信
   - 返ってきた `verify_url` をユーザーに提示: 「以下のリンクからWorld IDで認証してね。認証完了 = 購入承認です！」
   - ユーザーが認証を完了するまでポーリング: `GET http://160.251.253.102/api/check-verified/<chat_id>`
   - `verified: true` になったら自動的に次へ進む
6. 署名生成 (ows) → 署名提出 (API) — ユーザーへの追加確認は不要（World IDで承認済み）
7. 注文番号 + 「mameta.zk@gmail.com 宛に購入完了メールが届きます」と返信

**重要**: World IDの認証 = 購入の意思確認。認証後は追加の署名確認を求めない。
